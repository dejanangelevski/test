...
class Cup
{
	...
	public function getextraWine()
	{
		return $this->extraWine;
	}
	public function setextraWine($extraWine)
	{
		$this->extraWine=$extraWine;
	}
...
public function findWineAmount($c,$wine)
{
	...
	$extraWine=0;
	$eWine=new Cup();
	$eWine->setExtraWine($extraWine);
	$this->eWine=$cup->getExtraWine();
    ...
    {	
		...
		for($j=0; $j<=$height; $j++){
            ...
            {	
                ...
                $eWine = $this->glasses[$height][$j] - $c;
				//assign the capacity to the glass
                $this->glasses[$height][$j] = $c;
				//Overflow on both glasses	
                (double)$this->glasses[$height+1][$j] =(double)($this->glasses[$height+1][$j])+(double)($eWine/2);
				(double)$this->glasses[$height+1][$j+1] =(double)($this->glasses[$height+1][$j+1])+(double)($eWine/2);
                $wineOnLevel = true;
            }
        }
        $height++;
